#!/usr/bin/env python3
"""L4D2 LLM Bot - Prompt Builder (v1.2)"""
...
class PromptBuilder:
    ...
    def build(self, state: Dict[str, Any], last_action: Optional[Dict] = None, 
        ...
        parts.append(f"\n[Self] {bot_name}: HP={bot_hp}{(' ['+','.join(status_parts)+']') if status_parts else ''}")
        parts.append(f"  Primary: {self._clean_weapon_name(weapons.get('primary','none'))} (ammo:{ammo})")
        ...
        if teammates:
            ...
            for m in teammates:
                ...
                if m.get("pinned"): status.append("PINNED")
                status_str = f" [{','.join(status)}]" if status else ""
                parts.append(f"  {m.get('name','?')}: HP={m.get('hp',0)}, dist={m.get('dist',0):.0f}{status_str}")
        else:
        ...
        parts.append('\nReply JSON: {"action":"...","params":{},"priority":"critical|high|medium|low","reason":"..."}')
